...
The signed manifest references the primary Agent identity artwork through `icon` and the home poster through `heroImage`. Tutti projects the icon to Agent selectors, conversation rows, Message Center, and mentions. Keep the canonical icon background transparent so monochrome mask surfaces preserve the glyph instead of collapsing it into a solid block. Keep each packaged image at or below 256 KiB and replace it deliberately when branding changes.
